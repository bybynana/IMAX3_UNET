...
printf("DR..DMA phase=%x i=%d j=%d dirty=%x\n", phase, i, j, emax6.lmmd[i][j]);
  ...
  if      (phase == 1) { /* drain */
    if      (mode==0 && lmmo_stat==12 && !lmm_ready && lmmc_stat!=13 && (emax6.lmmd[m][j]&1<<c)) { mark=1;emax6.lmmd[m][j]&=~(1<<c);dmadr=lmmiop->top;dmlen=lmmiop->len;dmnxt=lmmiop1->top;dmrw=1;}/* ●2 lmw&!lmd drain */
