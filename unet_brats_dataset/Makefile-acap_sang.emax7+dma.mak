...
# all:		$(PROGRAM)
all: cnn3d-acap_sang_CONV${ARG_CONV}_RMGRP${ARG_RMGRP}.emax7+dma

clean:;		rm -f *.o core *~ *-mark.c *-cppo.c *-emax7.c *-emax7s?.c

cnn3d-acap_sang_CONV${ARG_CONV}_RMGRP${ARG_RMGRP}.emax7+dma: cnn3d+rmm_sang-emax7.o
...
cnn3d+rmm_sang-emax7.c:	cnn3d+rmm_sang.c
		...
		$(CPP) -DNCHIP=1 -DIMAP=2 -DCONV_SELECT=${ARG_CONV} -DRMGRP=${ARG_RMGRP} $(OPTION) $<-mark.c > $<-cppo.c
		$(PROJTOP)/src/conv-c2d/conv-c2d $<-cppo.c

###ARG_CONV=${i} ARG_RMGRP=${j}
