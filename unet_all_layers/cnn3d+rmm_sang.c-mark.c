...
imax(int LANE) {
  ...
  for (ztop=1; ztop<D-1; ztop++) { /* depth */
    ...
    ip0[k][ 1] = ip[k]+(ztop-1)*H*W+(top-1)*W+1+0; 
    ip0[k][ 2] = ip[k]+(ztop-1)*H*W+(top-1)*W+1+1;
	  ip0[k][ 3] = ip[k]+(ztop-1)*H*W+(top+0)*W+1-1; 
    ip0[k][ 4] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+0; 
    ip0[k][ 5] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+1;
	  ip0[k][ 6] = ip[k]+(ztop-1)*H*W+(top+1)*W+1-1; 
    ip0[k][ 7] = ip[k]+(ztop-1)*H*W+(top+1)*W+1+0; 
    ip0[k][ 8] = ip[k]+(ztop-1)*H*W+(top+1)*W+1+1;

	  ip0[k][ 9] = ip[k]+(ztop+0)*H*W+(top-1)*W+1-1; 
    ip0[k][10] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+0;
    ip0[k][11] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+1;
	  ip0[k][12] = ip[k]+(ztop+0)*H*W+(top+0)*W+1-1; 
    ip0[k][13] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+0; 
    ip0[k][14] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+1;
	  ip0[k][15] = ip[k]+(ztop+0)*H*W+(top+1)*W+1-1; 
    ip0[k][16] = ip[k]+(ztop+0)*H*W+(top+1)*W+1+0; 
    ip0[k][17] = ip[k]+(ztop+0)*H*W+(top+1)*W+1+1;

	  ip0[k][18] = ip[k]+(ztop+1)*H*W+(top-1)*W+1-1; 
    ip0[k][19] = ip[k]+(ztop+1)*H*W+(top-1)*W+1+0; 
    ip0[k][20] = ip[k]+(ztop+1)*H*W+(top-1)*W+1+1;
	  ip0[k][21] = ip[k]+(ztop+1)*H*W+(top+0)*W+1-1; 
    ip0[k][22] = ip[k]+(ztop+1)*H*W+(top+0)*W+1+0; 
    ip0[k][23] = ip[k]+(ztop+1)*H*W+(top+0)*W+1+1;
	  ip0[k][24] = ip[k]+(ztop+1)*H*W+(top+1)*W+1-1; 
    ip0[k][25] = ip[k]+(ztop+1)*H*W+(top+1)*W+1+0; 
    ip0[k][26] = ip[k]+(ztop+1)*H*W+(top+1)*W+1+1;

	  ip1[k][ 0] = ip[k]+(ztop-1)*H*W+(top-1)*W+1+1; 
    ip1[k][ 1] = ip[k]+(ztop-1)*H*W+(top-1)*W+1+2; 
    ip1[k][ 2] = ip[k]+(ztop-1)*H*W+(top-1)*W+1+3;
	  ip1[k][ 3] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+1; 
    ip1[k][ 4] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+2; 
    ip1[k][ 5] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+3;
	  ip1[k][ 6] = ip[k]+(ztop-1)*H*W+(top+1)*W+1+1; 
    ip1[k][ 7] = ip[k]+(ztop-1)*H*W+(top+1)*W+1+2; 
    ip1[k][ 8] = ip[k]+(ztop-1)*H*W+(top+1)*W+1+3;

	  ip1[k][ 9] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+1; 
    ip1[k][10] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+2; 
    ip1[k][11] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+3;
	  ip1[k][12] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+1; 
    ip1[k][13] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+2; 
    ip1[k][14] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+3;
	  ip1[k][15] = ip[k]+(ztop+0)*H*W+(top+1)*W+1+1; 
    ip1[k][16] = ip[k]+(ztop+0)*H*W+(top+1)*W+1+2; 
    ip1[k][17] = ip[k]+(ztop+0)*H*W+(top+1)*W+1+3;

	  ip1[k][18] = ip[k]+(ztop+1)*H*W+(top-1)*W+1+1; 
    ip1[k][19] = ip[k]+(ztop+1)*H*W+(top-1)*W+1+2; 
    ip1[k][20] = ip[k]+(ztop+1)*H*W+(top-1)*W+1+3;
	  ip1[k][21] = ip[k]+(ztop+1)*H*W+(top+0)*W+1+1; 
    ip1[k][22] = ip[k]+(ztop+1)*H*W+(top+0)*W+1+2; 
    ip1[k][23] = ip[k]+(ztop+1)*H*W+(top+0)*W+1+3;
	  ip1[k][24] = ip[k]+(ztop+1)*H*W+(top+1)*W+1+1; 
    ip1[k][25] = ip[k]+(ztop+1)*H*W+(top+1)*W+1+2; 
    ip1[k][26] = ip[k]+(ztop+1)*H*W+(top+1)*W+1+3;
	}
...
/-EMAX5AB-/ cnn 0
/-EMAX5AS-/     /*3*/ for (CHIP=0; CHIP<NCHIP; CHIP++) { /* output channels are parallelized by multi-chip (OC/#chip) */
/-EMAX5AS-/       /*2*/ for (INIT1=1,LOOP1=""RMGRP"",rofs=""0-H*4""; LOOP1--; INIT1=0) {            /* stage#0 *//* mapped to FOR() on BR[63][1][0] */
/-EMAX5AS-/         /*1*/ for (INIT0=1,LOOP0=""(W-2)/2"",cofs=""0-4""; LOOP0--; INIT0=0) {          /* stage#0 *//* mapped to FOR() on BR[63][0][0] */
/-EMAX5AS-/     exe(OP_ADD,    &cofs, INIT0?cofs:cofs, EXP_H3210, 8, EXP_H3210, 0LL, EXP_H3210, OP_AND, 0x00000000ffffffffLL, OP_NOP, 0LL);/* stage#0: cof+=8 */
/-EMAX5AS-/ 		exe(OP_ADD,    &rofs, rofs, EXP_H3210, INIT0?H*4:0,  EXP_H3210, 0LL, EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);           /* stage#0: rof+=H*4(init) or 0(not init) */
/-EMAX5AS-/ 		exe(OP_ADD,    &oofs, rofs, EXP_H3210, cofs, EXP_H3210, 0LL, EXP_H3210, OP_AND, 0x00000000ffffffffLL, OP_NOP, 0LL);  /* stage#1: oof=rof+cof */
/-EMAX5AS-/     // printf("rofs=%d, cofs=%d, oofs=%d\n", rofs, cofs, oofs);
/-EMAX5AS-/     // printf("kp0[0][CHIP]=%p, kp1[0][CHIP]=%p, kp2[0][CHIP]=%p, kp3[0][CHIP]=%p\n", kp0[0][CHIP], kp1[0][CHIP], kp2[0][CHIP], kp3[0][CHIP]);
/-EMAX5AS-/     // printf("ip1[0][0]=%p, ip0[0][0]=%p\n", ip1[0][0], ip0[0][0]);
/-EMAX5AS-/     // printf("op0[CHIP]=%p, op1[CHIP]=%p, op2[CHIP]=%p, op3[CHIP]=%p\n", op0[CHIP], op1[CHIP], op2[CHIP], op3[CHIP]);
/-EMAX5AS-/ 
/-EMAX5AS-/ 		mop(OP_LDWR,   1, &BR[2][0][1],  (Ull)kp0[0][CHIP], 0LL, MSK_D0, (Ull)ker, IC*OC*K*K*K, 0, 0, (Ull)NULL, IC*OC*K*K*K); /* stage#2 */
/-EMAX5AS-/ 		mop(OP_LDWR,   1, &BR[2][0][0],  (Ull)kp1[0][CHIP], 0LL, MSK_D0, (Ull)ker, IC*OC*K*K*K, 0, 0, (Ull)NULL, IC*OC*K*K*K); /* stage#2 */
/-EMAX5AS-/ 		mop(OP_LDWR,   1, &BR[2][1][1],  (Ull)kp2[0][CHIP], 0LL, MSK_D0, (Ull)ker, IC*OC*K*K*K, 0, 0, (Ull)NULL, IC*OC*K*K*K); /* stage#2 */
/-EMAX5AS-/ 		mop(OP_LDWR,   1, &BR[2][1][0],  (Ull)kp3[0][CHIP], 0LL, MSK_D0, (Ull)ker, IC*OC*K*K*K, 0, 0, (Ull)NULL, IC*OC*K*K*K); /* stage#2 10KB */
/-EMAX5AS-/ 		mop(OP_LDR,    1, &BR[2][2][1],  (Ull)ip1[0][0], oofs, MSK_W0, (Ull)it[0][0], W*(RMGRP+2), 0, 0, (Ull)NULL, W*(RMGRP+2)); /* stage#2 8KB *//* unaligned load */
/-EMAX5AS-/ 		mop(OP_LDR,    1, &BR[2][2][0],  (Ull)ip0[0][0], oofs, MSK_W0, (Ull)it[0][0], W*(RMGRP+2), 0, 0, (Ull)NULL, W*(RMGRP+2)); /* stage#2 8KB *//* unaligned load */
/-EMAX5AS-/ 		exe(OP_FML, &AR[3][0], BR[2][2][0], EXP_H3210, BR[2][0][1], EXP_H1010, 0LL, EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL); /* stage#3 */
/-EMAX5AS-/ 		exe(OP_FML, &AR[3][1], BR[2][2][0], EXP_H3210, BR[2][0][0], EXP_H1010, 0LL, EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL); /* stage#3 */
/-EMAX5AS-/ 		exe(OP_FML, &AR[3][2], BR[2][2][0], EXP_H3210, BR[2][1][1], EXP_H1010, 0LL, EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL); /* stage#3 */
/-EMAX5AS-/ 		exe(OP_FML, &AR[3][3], BR[2][2][0], EXP_H3210, BR[2][1][0], EXP_H1010, 0LL, EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL); /* stage#3 */
/-EMAX5AS-/ 
/-EMAX5AS-/ 		cnn_core1( 3, 0, 0,  4LL, 1,  4); // r,i,j,ofs,k,rp1
/-EMAX5AS-/ 		cnn_core1( 4, 0, 0,  8LL, 2,  5);
/-EMAX5AS-/ 		cnn_core1( 5, 0, 0, 12LL, 3,  6);
/-EMAX5AS-/ 		cnn_core1( 6, 0, 0, 16LL, 4,  7);
/-EMAX5AS-/ 		cnn_core1( 7, 0, 0, 20LL, 5,  8);
/-EMAX5AS-/ 		cnn_core1( 8, 0, 0, 24LL, 6,  9);
/-EMAX5AS-/ 		cnn_core1( 9, 0, 0, 28LL, 7, 10);
/-EMAX5AS-/ 		cnn_core1(10, 0, 0, 32LL, 8, 11);
/-EMAX5AS-/ 
/-EMAX5AS-/ 		cnn_core1(11, 0, 1, 36LL, 9, 12);
/-EMAX5AS-/ 		cnn_core1(12, 0, 1, 40LL,10, 13);
/-EMAX5AS-/ 		cnn_core1(13, 0, 1, 44LL,11, 14);
/-EMAX5AS-/ 		cnn_core1(14, 0, 1, 48LL,12, 15);
/-EMAX5AS-/ 		cnn_core1(15, 0, 1, 52LL,13, 16);
/-EMAX5AS-/ 		cnn_core1(16, 0, 1, 56LL,14, 17);
/-EMAX5AS-/ 		cnn_core1(17, 0, 1, 60LL,15, 18);
/-EMAX5AS-/ 		cnn_core1(18, 0, 1, 64LL,16, 19);
/-EMAX5AS-/ 		cnn_core1(19, 0, 1, 68LL,17, 20);
/-EMAX5AS-/ 
/-EMAX5AS-/ 		cnn_core1(20, 0, 2, 72LL,18, 21);
/-EMAX5AS-/ 		cnn_core1(21, 0, 2, 76LL,19, 22);
/-EMAX5AS-/ 		cnn_core1(22, 0, 2, 80LL,20, 23);
/-EMAX5AS-/ 		cnn_core1(23, 0, 2, 84LL,21, 24);
/-EMAX5AS-/ 		cnn_core1(24, 0, 2, 88LL,22, 25);
/-EMAX5AS-/ 		cnn_core1(25, 0, 2, 92LL,23, 26);
/-EMAX5AS-/ 		cnn_core1(26, 0, 2, 96LL,24, 27);
/-EMAX5AS-/ 		cnn_core1(27, 0, 2,100LL,25, 28);
/-EMAX5AS-/ 		cnn_core1(28, 0, 2,104LL,26, 29);
#if (IMAP==1)
/-EMAX5AS-/ 		cnn_final(29,     30);
#endif
#if (IMAP>1)
/-EMAX5AS-/ 		cnn_core1(29, 1, 0,  0LL, 0, 30);
/-EMAX5AS-/ 		cnn_core1(30, 1, 0,  4LL, 1, 31);
/-EMAX5AS-/ 		cnn_core1(31, 1, 0,  8LL, 2, 32);
/-EMAX5AS-/ 		cnn_core1(32, 1, 0, 12LL, 3, 33);
/-EMAX5AS-/ 		cnn_core1(33, 1, 0, 16LL, 4, 34);
/-EMAX5AS-/ 		cnn_core1(34, 1, 0, 20LL, 5, 35);
/-EMAX5AS-/ 		cnn_core1(35, 1, 0, 24LL, 6, 36);
/-EMAX5AS-/ 		cnn_core1(36, 1, 0, 28LL, 7, 37);
/-EMAX5AS-/ 		cnn_core1(37, 1, 0, 32LL, 8, 38);
/-EMAX5AS-/ 
/-EMAX5AS-/ 		cnn_core1(38, 1, 1, 36LL, 9, 39);
/-EMAX5AS-/ 		cnn_core1(39, 1, 1, 40LL,10, 40);
/-EMAX5AS-/ 		cnn_core1(40, 1, 1, 44LL,11, 41);
/-EMAX5AS-/ 		cnn_core1(41, 1, 1, 48LL,12, 42);
/-EMAX5AS-/ 		cnn_core1(42, 1, 1, 52LL,13, 43);
/-EMAX5AS-/ 		cnn_core1(43, 1, 1, 56LL,14, 44);
/-EMAX5AS-/ 		cnn_core1(44, 1, 1, 60LL,15, 45);
/-EMAX5AS-/ 		cnn_core1(45, 1, 1, 64LL,16, 46);
/-EMAX5AS-/ 		cnn_core1(46, 1, 1, 68LL,17, 47);
/-EMAX5AS-/ 
/-EMAX5AS-/ 		cnn_core1(47, 1, 2, 72LL,18, 48);
/-EMAX5AS-/ 		cnn_core1(48, 1, 2, 76LL,19, 49);
/-EMAX5AS-/ 		cnn_core1(49, 1, 2, 80LL,20, 50);
/-EMAX5AS-/ 		cnn_core1(50, 1, 2, 84LL,21, 51);
/-EMAX5AS-/ 		cnn_core1(51, 1, 2, 88LL,22, 52);
/-EMAX5AS-/ 		cnn_core1(52, 1, 2, 92LL,23, 53);
/-EMAX5AS-/ 		cnn_core1(53, 1, 2, 96LL,24, 54);
/-EMAX5AS-/ 		cnn_core1(54, 1, 2,100LL,25, 55);
/-EMAX5AS-/ 		cnn_core1(55, 1, 2,104LL,26, 56);
#endif
#if (IMAP==2)
/-EMAX5AS-/ 		cnn_final(56,     57);
#endif
/-EMAX5AS-/               }
/-EMAX5AS-/             }
/-EMAX5AS-/           }
