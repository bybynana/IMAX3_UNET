...
#ifndef CONV_SELECT
#define DECONV_SELECT 8
#endif

#if CONV_SELECT == 8
#define IN_PATH "Data_Create_Test/input_conv8.bin"
#define KER_PATH "Data_Create_Test/input_conv8_ker.bin"
#define OUT_PATH "RMMA_imax_out/imax_conv8.bin"
#define IC 512
#define OC 512
#define D 20
#define H 28
...
// #if DECONV_SELECT == 1
// #define IN_PATH "deconv3d_bins/out_deconv1.bin"
// #define KER_PATH "deconv3d_bins/deconv1_weight.bin"
// #define OUT_PATH "imax_out/imax_deconv1.bin"
// #define IC 1024
// #define OC 512
// #define D 5
// #define H 17
...
// #if DECONV_SELECT == 2
// #define IN_PATH "deconv3d_bins/in_deconv2.bin"
// #define KER_PATH "deconv3d_bins/deconv2_weight.bin"
// #define OUT_PATH "imax_out/imax_deconv2.bin"
// #define IC 512
// #define OC 256
// #define D 5
// #define H 34
...
// #if DECONV_SELECT == 3
// #define IN_PATH "deconv3d_bins/in_deconv3.bin"
// #define KER_PATH "deconv3d_bins/deconv3_weight.bin"
// #define OUT_PATH "imax_out/imax_deconv3.bin"
// #define IC 256
// #define OC 128
// #define D 10
// #define H 68
...
// #if DECONV_SELECT == 4
// #define IN_PATH "deconv3d_bins/in_deconv4.bin"
// #define KER_PATH "deconv3d_bins/deconv4_weight.bin"
// #define OUT_PATH "imax_out/imax_deconv4.bin"
// #define IC 128
// #define OC 64
// #define D 20
// #define H 269
...
imax(int LANE) {
  ...
  for (ztop=1; ztop<D-1; ztop++) { /* depth */
    ...
    ip0[k][ 2] = ip[k]+(ztop-1)*H*W+(top+0)*W+1-1; 
    ip0[k][ 3] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+0; 

    ip0[k][ 4] = ip[k]+(ztop+0)*H*W+(top-1)*W+1-1; 
    ip0[k][5] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+0;
    ip0[k][6] = ip[k]+(ztop+0)*H*W+(top+0)*W+1-1; 
    ip0[k][7] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+0; 


    ip1[k][ 0] = ip[k]+(ztop-1)*H*W+(top-1)*W+1+1; 
    ip1[k][ 1] = ip[k]+(ztop-1)*H*W+(top-1)*W+1+2; 
    ip1[k][ 2] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+1; 
    ip1[k][ 3] = ip[k]+(ztop-1)*H*W+(top+0)*W+1+2; 

    ip1[k][4] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+1; 
    ip1[k][5] = ip[k]+(ztop+0)*H*W+(top-1)*W+1+2; 
    ip1[k][6] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+1; 
    ip1[k][7] = ip[k]+(ztop+0)*H*W+(top+0)*W+1+2; 
	  //it[0][k] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1-1; // stride = 2
	  //it[1][k] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1-1; // stride = 2
	  //it[2][k] = ip[k]+((ztop+1)/2)*H*W+((top-1)/2)*W+1-1; // stride = 2

    // ip0[k][0] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1-1; 
    // ip0[k][1] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+0;
    // ip0[k][2] = ip[k]+((ztop-1)/2)*H*W+((top+0)/2)*W+1-1; 
    // ip0[k][3] = ip[k]+((ztop-1)/2)*H*W+((top+0)/2)*W+1+0; 
    // ip0[k][4] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1-1; 
    // ip0[k][5] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+0;
    // ip0[k][6] = ip[k]+((ztop+0)/2)*H*W+((top+0)/2)*W+1-1; 
    // ip0[k][7] = ip[k]+((ztop+0)/2)*H*W+((top+0)/2)*W+1+0; 

    // ip1[k][ 0] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+1; 
    // ip1[k][ 1] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+2; 
    // ip1[k][ 3] = ip[k]+((ztop-1)/2)*H*W+((top+0)/2)*W+1+1; 
    // ip1[k][ 4] = ip[k]+((ztop-1)/2)*H*W+((top+0)/2)*W+1+2;
    // ip1[k][ 5] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+1; 
    // ip1[k][6] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+2; 
    // ip1[k][7] = ip[k]+((ztop+0)/2)*H*W+((top+0)/2)*W+1+1;
    // ip1[k][8] = ip[k]+((ztop+0)/2)*H*W+((top+0)/2)*W+1+2; 

    //ip0[k][0] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1-1; // stride = 2
    //ip0[k][1] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+0; // stride = 2
    //ip0[k][2] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+1; // stride = 2
	  //ip0[k][3] = ip[k]+((ztop-1)/2)*H*W+((top+0)/2)*W+1-1; // stride = 2
	  //ip0[k][4] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1-1; // stride = 2
    //ip0[k][5] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+0; // stride = 2
    //ip0[k][6] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+1; // stride = 2
	  //ip0[k][7] = ip[k]+((ztop+0)/2)*H*W+((top+0)/2)*W+1-1; // stride = 2
	  //ip1[k][0] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+1; // stride = 2
    //ip1[k][1] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+2; // stride = 2
    //ip1[k][2] = ip[k]+((ztop-1)/2)*H*W+((top-1)/2)*W+1+3; // stride = 2
	  //ip1[k][3] = ip[k]+((ztop-1)/2)*H*W+((top+0)/2)*W+1+1; // stride = 2
	  //ip1[k][4] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+1; // stride = 2
    //ip1[k][5] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+2; // stride = 2
    //ip1[k][6] = ip[k]+((ztop+0)/2)*H*W+((top-1)/2)*W+1+3; // stride = 2
	  //ip1[k][7] = ip[k]+((ztop+0)/2)*H*W+((top+0)/2)*W+1+1; // stride = 2
	}
	...
	for (oc=0; oc<OC/NCHIP; oc+=W_) { /* set output channel */
	  ...
	  for (CHIP=0; CHIP<NCHIP; CHIP++) { /* output channels are parallelized by multi-chip (OC/#chip) */
	    ...
	    op0[CHIP] = out1+(choc+0)* D*H*W+ztop*H*W+top*W+0; 
      op1[CHIP] = out1+(choc+1)* D*H*W+ztop*H*W+top*W+0; 
      op2[CHIP] = out1+(choc+2)* D*H*W+ztop*H*W+top*W+0; 
      op3[CHIP] = out1+(choc+3)* D*H*W+ztop*H*W+top*W+0;
	    ot0[CHIP] = out1+(choc+0)* D*H*W+ztop*H*W+top*W+0; 
      ot1[CHIP] = out1+(choc+1)* D*H*W+ztop*H*W+top*W+0; 
      ot2[CHIP] = out1+(choc+2)* D*H*W+ztop*H*W+top*W+0; 
      ot3[CHIP] = out1+(choc+3)* D*H*W+ztop*H*W+top*W+0;
	  }

#define deconv_core1(r, i, j, ofs, k, rp1) \
...
//EMAX5A begin cnn mapdist=0
    ...
    exe(OP_ADD,    &cofs, INIT0?cofs:cofs, EXP_H3210, 8, EXP_H3210, 0LL, EXP_H3210, OP_AND, 0x00000000ffffffffLL, OP_NOP, 0LL);/* stage#0: cof+=8 */
		...
		deconv_core1( 3, 0, 0,  4LL, 1,  4); // r,i,j,ofs,k,rp1
		deconv_core1( 4, 0, 0,  8LL, 2,  5);
		deconv_core1( 5, 0, 0, 12LL, 3,  6);

		deconv_core1( 6, 0, 0, 16LL, 4,  7);
		deconv_core1( 7, 0, 0, 20LL, 5,  8);
		deconv_core1( 8, 0, 0, 24LL, 6,  9);
		deconv_core1( 9, 0, 0, 28LL, 7, 10);
#if (IMAP==1)
		deconv_final(10,     11);
#endif
#if (IMAP>1)
		deconv_core1(10, 1, 0,  0LL, 0, 11);
		deconv_core1(11, 1, 0,  4LL, 1, 12);
		deconv_core1(12, 1, 0,  8LL, 2, 13);
		deconv_core1(13, 1, 0, 12LL, 3, 14);

		deconv_core1(14, 1, 0, 16LL, 4, 15);
		deconv_core1(15, 1, 0, 20LL, 5, 16);
		deconv_core1(16, 1, 0, 24LL, 6, 17);
		deconv_core1(17, 1, 0, 28LL, 7, 18);
