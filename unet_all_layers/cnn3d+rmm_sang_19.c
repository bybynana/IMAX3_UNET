...
#define OUT_PATH "imax_out/imax_conv19_arm.bin"
#define IC 64
#define OC 4
#define D 20
#define H 269
...
conv3d_arm() {
    ...
    // output.data = (float *) malloc(output.num * output.channel * output.depth * output.height * output.width * sizeof(float));sang-imax-unet-all-layers/IMAX-CPU19/proj-arm64/sample/mm_cnn_lf/cnn3d+rmm_zhu.c
  ...
  for (int d = 0; d < D; d++) {
    for (int h = 0; h < H; h++) {
      for (int w = 0; w < W; w++) {
            ...
            for (int fh = 0; fh < K; fh++) {
              for (int fw = 0; fw < K; fw++) {
                ...
                if (d_in >= 0 && d_in < D &&
                    h_in >= 0 && h_in < H &&
                    w_in >= 0 && w_in < W) {
                  // int t_idx = get_tensor_index(input, c, d_in, h_in, w_in);
                  // int f_idx = get_tensor_index(kernels, k, c, fd, fh, fw);
                  // value += *(float*)&in[IC*H*W*D*k + IC*H*W*d + IC*W*h + w] * *(float*)&ker[OC*IC*K*K*K*k + OC*IC*K*K*fd + OC*IC*K*fh + OC*IC*fw + OC*c];
                  value += *(float*)&in[c*D*H*W + d*H*W + h*W + w] * *(float*)&ker[k*IC*K*K*K + c*K*K*K + fd*K*K + fh*K + fw];
        ...
        // *(float*)&out1[OC*H*W*D*k + OC*H*W*d + OC*W*h + w] = value;
        *(float*)&out1[k*D*H*W + d*H*W + h*W + w] = value;
        // (double)*(float*)&out0[oc*D*H*W + dep*H*W + row*W + col]
